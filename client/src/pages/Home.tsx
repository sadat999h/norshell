  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f5f3ef] text-[#211c18]">
      ...
      <main>
        ...
        {highlights.length ? <section className="bg-[#3e2c22] px-5 py-24 text-[#f5f1eb] sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-7xl">{highlights.map((highlight, index) => <article key={highlight.id} className={`grid gap-10 ${index ? "mt-24 lg:grid-cols-[.85fr_1.15fr]" : "lg:grid-cols-[1.15fr_.85fr]"} lg:items-center lg:gap-20`}><div className={index ? "lg:order-2" : ""}><img src={highlight.imageUrl} alt={highlight.altText || highlight.heading} className="aspect-[4/5] w-full object-cover" /></div><div className={index ? "lg:order-1" : ""}><p className="ns-label text-[#d5b8a2]">{highlight.eyebrow || "Norshell"}</p><h2 className="ns-display mt-5 max-w-md text-5xl leading-[.93] sm:text-6xl">{highlight.heading}</h2><p className="mt-7 max-w-sm text-[15px] leading-7 text-[#eee4db]/75">{highlight.body}</p><a href="#story" className="ns-text-link mt-8 text-[#f5f1eb]">Our point of view <CircleArrowRight size={16} /></a></div></article>)}</div></section> : null}

        {story ? <section id="story" className="grid bg-[#e4ded6] lg:grid-cols-2"><div className="min-h-[60vh] overflow-hidden"><img src={story.imageUrl} alt={story.altText || story.heading} className="h-full w-full object-cover" /></div><div className="flex items-center px-6 py-24 sm:px-12 lg:px-20"><div className="max-w-lg"><p className="ns-label">Norshell, in brief</p><blockquote className="mt-7 font-serif text-4xl leading-[1.02] tracking-[-.045em] sm:text-5xl">“{story.quote}”</blockquote><p className="mt-5 text-xs uppercase tracking-[.16em] text-[#80614d]">{story.attribution || "Norshell"}</p><h2 className="ns-display mt-16 text-4xl sm:text-5xl">{story.heading}</h2><p className="mt-6 text-[15px] leading-7 text-[#5d5048]">{story.body}</p></div></div></section> : null}
